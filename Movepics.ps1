...
{	
	...
	{
	#enter source path
	$sourcePath = Read-Host "Enter the Source Path[attempt #$inputcount] " 
	$testReadPath = Test-Path $sourcepath 
	if($testReadPath) #varies output based on whether the path exists or not
		{
		...
		$destPath = Read-Host "Enter the Destination Path[attempt #$inputcount] " 
	...
	if($testReadDestPath)
		...
		Write-Host "the results, if you are in a loop, break here`n"
		Start-Sleep -Seconds 2
		$inputcount++
	}
...
#Write-Host "Gathering Information on objects within $sourcePath"
#the actual act of gathing info on all the objects in the directory.
